...
<head>
    ...
    <title>Product Search - Skin Care Website</title>
...
<body>
    ...
    <main class="main">
        <section class="section container">
            <h2 class="section__title">Product Search</h2>
            
            <div class="search-container">
                <form action="product_search.php" method="GET" class="search-form">
                    <input type="text" name="txt_product" class="search-input" placeholder="Enter product name..." required>
                ...
                <div class="results-container">
                    <?php
                    // Process search if product name is provided
                    if (isset($_GET['txt_product']) && !empty($_GET['txt_product'])) {
                        ...
                        if (is_array($result) && !empty($result)) {
                            foreach ($result as $record) {
                                $productXml = $XMLDocument->addChild('Product');
                                $productXml->addChild('Id', isset($record['id']) ? $record['id'] : '');
                                $productXml->addChild('Name', isset($record['name']) ? htmlspecialchars($record['name'], ENT_XML1) : '');
                                $productXml->addChild('Category', isset($record['category']) ? htmlspecialchars($record['category'], ENT_XML1) : '');
                                $productXml->addChild('Price', isset($record['price']) ? $record['price'] : '');
                                $productXml->addChild('DiscountPrice', isset($record['discount_price']) ? $record['discount_price'] : '');
                                
                                $desc = $productXml->addChild('Description');
                                if (isset($record['description'])) {
                                    $dom = dom_import_simplexml($desc);
                                    $cdata = $dom->ownerDocument->createCDATASection($record['description']);
                                    $dom->appendChild($cdata);
                                }
                                
                                $img = $productXml->addChild('Image');
                                if (isset($record['image'])) {
                                    $dom = dom_import_simplexml($img);
                                    $cdata = $dom->ownerDocument->createCDATASection($record['image']);
                                    $dom->appendChild($cdata);
                                }
                                
                                $productXml->addChild('IsNew', isset($record['is_new']) && $record['is_new'] ? 'true' : 'false');
                                $productXml->addChild('IsBest', isset($record['is_best']) && $record['is_best'] ? 'true' : 'false');
                            }
